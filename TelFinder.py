...
#############
parser.add_argument('-f', '--format', required=True, help='format of input genome. fa means fasta; fq means fastq; kmer means kmer candidates(for mutation analysis)')
...
if input_format == 'fa':
	...
	chrEndAll = GetChrEnd(int(seq_len),species,input_file,out_dir)
	which = ['five','three']
	index = which.index(args.end)
	out_file = species+'_'+which[index]+'_info'
	out1 = open(os.path.join(out_dir,out_file),'w+')
	out1.write('chr\tkmer_size\tkmer\ttotal_repeat_number\tgap_number\tgap_dis\tgap_base\n')
	chrEndSeq =chrEndAll[index]
	...
	out_dc1 = OutCandi(supp2,chr_motif)
	out_score = species+'_'+which[index]+'_score'
...
if input_format == 'kmer':
	...
	chr_motif, chr_seq = IndelSeq(input_file, motif)
	OutputMut(chr_motif, chr_seq, motif, info1, out_dir)
