...
import os
#########################
def GetChrEnd(len0,species,fasta_file,dir1):
    chrEnd1 = {}
    chrEnd2 = {}
    info1 = []
    ...
    for k1 in fas:
        ...
        if info1 == []:
            info1.append(id1[:2])
        if id1[:2] == info1[0]:
            chrEnd1[k1.id] = seq[:len0].upper()
            chrEnd2[k1.id] = seq[-(len0):].upper()
    chrEndSeq = [chrEnd1, chrEnd2]
    h1 = open(os.path.join(dir1,species+'_tmp'),'w+')
    for k1 in chrEnd1:
        h1.write('>'+k1+'\n'+chrEnd1[k1]+chrEnd2[k1]+'\n')
    h1.close()
    return chrEndSeq
